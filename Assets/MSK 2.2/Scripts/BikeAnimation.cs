...
public class BikeAnimation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(eventPoint.position, direction, out hit, 1.0f) && BikeScript.speed > 30)
        {
			if (hit.collider.transform.root != transform.root && hit.collider.tag!="RespawnPoint")
            {
                if (player.parent != null)
                {
                   ...
                   player.GetComponent<BikeAnimation>().enabled = false;
