...
public class BikeCamera : MonoBehaviour
{
	...
	private bool IsTouchCameraEnable = false;
    ...
    public class BikeUIClass
    {
		...
		public Image speedFillBarImage;
    ...
    public void CameraSwitch()
    {
		...
		{ 
			Switch = 1;
			HandAnimated.SetActive (false);
			StopCoroutine ("HandAnimation");
		...
		{
			if (!IsTouchCameraEnable) {
				GetComponent<Camera> ().enabled = false;
			//	Player.Instance.TouchCameraTarget.SetActive (true);
				IsTouchCameraEnable = true;		
				StartCoroutine ("HandAnimation");
			} else {
				GetComponent<Camera> ().enabled = true;
			//	Player.Instance.TouchCameraTarget.SetActive (false);
				IsTouchCameraEnable = false;
				HandAnimated.SetActive (false);
				StopCoroutine ("HandAnimation");
    ...
    public void ShowBikeUI()
    {
		...
		BikeUI.speedFillBarImage.fillAmount = (bikeScript.speed / bikeScript.bikeSetting.LimitForwardSpeed);
   ...
   //     BikerMan = Player.Instance.BikersPelvis[GameManager.Instance.SelectedBikerIndex].transform;
