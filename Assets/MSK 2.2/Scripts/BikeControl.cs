...
public enum ControlMode { simple = 1, touch = 2, tilt=3 }
...
public class BikeControl : MonoBehaviour
{
    ...
    private void OnEnable()
    {
       ...
       bikeSetting.bikerMan = bikeSetting.GangsterRigs[GameManager.Instance.CurrentGangster];
    }
    ...
    void Awake()
    {
        ...
        foreach (WheelComponent w in wheels)
        {
			...
			//float temp = col.center.y;
			col.center = new Vector3(0,col.center.y+0.1f,0);
		...
		//	bikeSetting.bikerMan = Player.Instance.BikersPelvis [GameManager.Instance.SelectedBikerIndex].transform;
		//}
    }
    ...
    void FixedUpdate()
    {
        ...
        if (activeControl)
        {
			...
			else if (controlMode == ControlMode.tilt)
			{
				...
				if (accelFwd != 0) { accel = accelFwd; } else { accel = accelBack; }
				steer = Mathf.MoveTowards(steer, Input.acceleration.x, 0.07f);
			}
...
//                    }
        ...
        Pitch = Mathf.Clamp(.5f + ((motorRPM - bikeSetting.idleRPM) / (bikeSetting.shiftUpRPM - bikeSetting.idleRPM)), 0.5f, 5.0f);

        shiftTime = Mathf.MoveTowards(shiftTime, 0.0f, 0.1f);

        if (Pitch == 1 || currentGear == 0)
        {
            bikeSounds.IdleEngine.volume = Mathf.Lerp(bikeSounds.IdleEngine.volume, 0.3f, 0.1f);
            bikeSounds.LowEngine.volume = Mathf.Lerp(bikeSounds.LowEngine.volume, 0.2f, 0.1f);
        ...
        else
        {
            ...
            bikeSounds.IdleEngine.volume = Mathf.Lerp(bikeSounds.IdleEngine.volume, .8f - Pitch, 0.1f);


            if ((Pitch > PitchDelay || accel > 0) && shiftTime == 0.0f)
            {
                bikeSounds.LowEngine.volume = Mathf.Lerp(bikeSounds.LowEngine.volume, 0.0f, 0.2f);
                bikeSounds.HighEngine.volume = Mathf.Lerp(bikeSounds.HighEngine.volume, 0.1f, 0.2f);
            }
            else
            {
                bikeSounds.LowEngine.volume = Mathf.Lerp(bikeSounds.LowEngine.volume, 0.1f, 0.2f);
